...
static int program_load_segment(proc* p, const elf_program* ph,
	...
	//Is this good?
	if(end_mem > p->original_break)
	{
		p->original_break = p->program_break = ((end_mem + PAGESIZE) & ~(PAGESIZE - 1));
		memcpy((uint8_t*)end, (void*)p->original_break, sizeof(uint8_t*));	//Find the last address
	}
