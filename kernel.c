...
void exception(x86_64_registers* reg) {
	...
	{
	    /* TODO */
		uint64_t offset = current->p_registers.reg_rdi;
		
		//Always these sanity checks, including overflows
		if( /*offset + current->program_break < current->program_break || */offset + current->program_break >= (MEMSIZE_VIRTUAL - PAGESIZE))
		{
			current->p_registers.reg_rax = (uintptr_t)-1;
            log_printf("Reached end of memory\n");
			break;
		}
		
	    current->p_registers.reg_rax = (uint64_t)current->program_break;
		current->program_break += offset;
		log_printf("Ptr: %p offset %u\n", current->program_break, offset);
	    break;
	}
    ...
    case INT_PAGEFAULT: 
	{
		...
		//Did we touch an address that hasn't yet been mapped?
		//1: Not past break
		//2: If we're out of physical mem, kill the process
		if(addr <= current->program_break && addr >= current->original_break && !(reg->reg_err & PFERR_PRESENT))	//Stack will be allocated by default
		{
			//map the page when touched
			//vamapping mapping = virtual_memory_lookup(current->p_pagetable, addr);
            addr &= ~(PAGESIZE -1);
            uintptr_t pa = (uintptr_t)palloc(current->p_pid);
            if(pa)
			    virtual_memory_map(current->p_pagetable, addr, pa, PAGESIZE,
                                        PTE_P | PTE_U | PTE_W, NULL);
            else
			{
                //current->p_state = P_BROKEN;    //What are we really supposed to do with page fault errors?
				syscall_exit();
				schedule();
			}
			break;
		}
        log_printf("Segmentation fault\n");
